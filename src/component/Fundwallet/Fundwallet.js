...
const Fundwallet = () => {
  ...
  const handleFundWallet = async (e) => {
    ...
    await axios
      .post(
        "http://localhost:9000/api/v1/transaction/fund-wallet",
        { pin, accountNumber, description, amount, accountBank },
        { headers }
      )
      .then((response) => {
        setApiSuccess(true);
    ...
    <ContainerFBGpd>
      ...
      <FormCard>
        <Form>
          {/* -----------------------------------Password Input - Div */}
          <FormDiv>
            <LabelInput htmlFor="login-input" style={{ marginTop: "0px" }}>
              Amount
            </LabelInput>
            <Input
              type="number"
            ...
            <ButtonP type="submit" onClick={handleFundWallet}>
              Continue
            </ButtonP>
          </FormDiv>
        </Form>
